...
savePath = './trained/model4-doucz/{}/{}-{}'.format(datasetName, training_steps, num_hidden)
if not os.path.exists(savePath):
    os.makedirs(savePath)

class RefactorDataset():
    def __init__(self, dn, testProportion = .15):
        self.data_before = np.genfromtxt('./input/{}/input-before.csv'.format(dn), delimiter=',')
        self.data_after = np.genfromtxt('./input/{}/input-after.csv'.format(dn), delimiter=',')
        self.lengths_before = np.genfromtxt('./input/{}/lengths-before.csv'.format(dn), delimiter=',')
        self.lengths_after = np.genfromtxt('./input/{}/lengths-after.csv'.format(dn), delimiter=',')
        self.max_seqlen = len(self.data_before[0])
        self.test_len = math.floor(len(self.data_before) * testProportion)
        self.batch_id = 0
        self.test_data = self.next(self.test_len)
        print("Input size: " + str(self.max_seqlen) + 'x' + str(len(self.data_before)) + " Test set: " + str(self.test_len))
        ...
        for step in range(1, training_steps+1):
            ...
            if step >= train_clever_from:
                ...
                if not cleverUsed:
                    ...
                    cleverUsed = True
            batch_before, batch_after, batch_seqlen_before, batch_seqlen_after, batch_labels = theDataset.next(batch_size)
