...
savePath = '../trained/code-fracz-645-local'
if not os.path.exists(savePath):
    os.makedirs(savePath)

class RefactorDataset():
    def __init__(self):
        self.data = np.genfromtxt('/usr/src/scqm-input/input.csv'.format(datasetName), delimiter=',')
        self.labels = np.genfromtxt('/usr/src/scqm-input/labels.csv'.format(datasetName), delimiter=',')
        self.seqlen = np.genfromtxt('/usr/src/scqm-input/lengths.csv'.format(datasetName), delimiter=',')
